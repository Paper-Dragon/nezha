...
install_agent() {
    ...
    if [ ! -n "$version" ]; then
        err "Fail to obtaine agent version, please check if the network can link https://api.github.com/repos/Paper-Dragon/agent/releases/latest"
        return 1
    else
    ...
    wget -t 2 -T 60 -O nezha-agent_linux_${os_arch}.zip https://${GITHUB_URL}/Paper-Dragon/agent/releases/download/${version}/nezha-agent_linux_${os_arch}.zip >/dev/null 2>&1
    if [ $? != 0 ]; then
