...
install_agent() {
    ...
    if [ ! -n "$version" ]; then
        err "获取版本号失败，请检查本机能否链接 https://api.github.com/repos/Paper-Dragon/agent/releases/latest"
        return 1
    else
    ...
    echo "正在下载监控端"
    if [ -z "$CN" ]; then
        NZ_AGENT_URL="https://${GITHUB_URL}/Paper-Dragon/agent/releases/download/${version}/nezha-agent_linux_${os_arch}.zip"
    else
        NZ_AGENT_URL="https://${GITHUB_URL}/Paper-Dragon/agent/releases/download/${version}/nezha-agent_linux_${os_arch}.zip"
    fi
